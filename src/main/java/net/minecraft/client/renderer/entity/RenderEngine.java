...
import net.minecraft.client.particle.TextureFX;
import net.minecraft.client.renderer.ImageBuffer;
import net.minecraft.client.settings.GameSettings;
import net.minecraft.util.GLAllocation;
import net.minecraft.util.thread.ThreadDownloadImageData;
...
public class RenderEngine {
	...
	public int getTexture(String var1) {
		...
		if(var2 != null) {
			...
			try {
				...
				if(var1.startsWith("##")) {
					this.setupTexture(this.unwrapImageByColumns(ImageIO.read(getTextureStream(var1.substring(2)))), var4);
				} else if(var1.startsWith("%%")) {
					this.clampTexture = true;
					this.setupTexture(
							ImageIO.read(getTextureStream(var1.substring(2))),
							var4
					);
					this.clampTexture = false;
				} else {
					this.setupTexture(
							ImageIO.read(getTextureStream(var1)),
							var4
					);
				}
				...
				throw new RuntimeException("Modern texture load failed: " + var1);
			}
		}
	}
	...
	public void refreshTextures() {
		...
		while(var1.hasNext()) {
			...
			try {
				if(var7.startsWith("##")) {
					var3 = this.unwrapImageByColumns(
							ImageIO.read(getTextureStream(var7.substring(2)))
					);
				} else if(var7.startsWith("%%")) {
					this.clampTexture = true;
					var3 = ImageIO.read(getTextureStream(var7.substring(2)));
					this.clampTexture = false;
				} else {
					var3 = ImageIO.read(getTextureStream(var7));
				}
	...
	// toru -> Added at 12:13 pm : 3/4/2026
	private InputStream getTextureStream(String path) {
		if(path.startsWith("/")) {
			path = path.substring(1);
		}

		InputStream STREAM = RenderEngine.class.getClassLoader().getResourceAsStream("resources/" + path);
		if(STREAM != null) return STREAM;

		return RenderEngine.class.getClassLoader().getResourceAsStream(path);
	}
}
